...
import { useTestPackageById, useUpdateTestPackage } from "@/hooks/use-test-packages-catalog";
...
export default function EditPackage() {
  ...
  // Fetch Package Data
  const { data: packageData, isLoading: isLoadingPackage } = useTestPackageById(id, tenant?.id);

  // Fetch All Tests
  const { data: testCatalog, isLoading: isLoadingTests } = useTestCatalog({
    q: "",
    page: 1,
    limit: 100,
    tenantId: tenant?.id,
  ...
  const testOptions = testList.map((t) => ({
    id: t.id,
    label: `${t.display_name || t.name} (₹${t.price?.toLocaleString() || 0})`,
    price: Number(t.price || 0),
  }));

  // Prefill Form + Selected Tests
  useEffect(() => {
    if (!packageData?.data || !testList.length || isInitialized) return;

    const pkg = packageData.data;
    
    // Get initial test IDs from package
    const initialTestIds = pkg.tests?.map((t) => t.id) || [];
    
    // Set form values
    setValue("name", pkg.name || "");
    setValue("code", pkg.code || "");
    setValue("description", pkg.description || "");
    setValue("offer_price", pkg.offer_price || pkg.price || 0);
    setValue("testsIds", initialTestIds);
    
    // Calculate total price from selected tests
    const selectedTestsData = testList.filter((t) => initialTestIds.includes(t.id));
    const total = selectedTestsData.reduce((sum, t) => sum + Number(t.price || 0), 0);
    setValue("total_price", total);
    
    // If offer price > total, reset it
    const currentOffer = pkg.offer_price || pkg.price || 0;
    if (currentOffer > total) {
      setValue("offer_price", total);
    }
    
    setIsInitialized(true);
  }, [packageData, testList, setValue, isInitialized]);
  ...
  useEffect(() => {
    ...
    // If offer_price is greater than new total, reset it to total
    if (offerPrice > total) {
      setValue("offer_price", total);
    }
  }, [selectedTests, setValue, offerPrice, isInitialized]);

  const updateMutation = useUpdateTestPackage();
  ...
  const onSubmit = async (data) => {
    ...
    // Strict validation: offer price must be <= total price
    if (offer > total) {
      toast.error(`Offer price (₹${offer.toLocaleString()}) cannot be greater than total price (₹${total.toLocaleString()})`);
      return;
    }

    // Optional: Warn if offer price is same as total (no discount)
    if (offer === total && total > 0) {
    ...
    try {
      const payload = {
        ...
        price: total,           // Original total price
        offer_price: offer,     // Discounted price (must be <= total)
  ...
  if (isLoadingPackage || isLoadingTests) {
    return (
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4 sm:p-6">
        <div className="max-w-4xl mx-auto py-6">
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-6 text-center">
            <p className="text-red-600 dark:text-red-400">Package not found</p>
            <Link href="/dashboard/test-price" className="mt-4 inline-block text-blue-600 dark:text-blue-400 hover:underline">
              Back to Packages
            </Link>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4 sm:p-6 transition-colors duration-200">
      <div className="max-w-4xl mx-auto py-6">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Update Test Package</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">Edit package details, pricing, and test selections</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <div key={test.id} className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                      ...
                      >
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          <span className="text-sm text-blue-600 dark:text-blue-400">₹{test.price?.toLocaleString()}</span>
                        </div>
          ...
          <div className="flex justify-between flex-col sm:flex-row gap-4 pt-4">
            ...
            <button
              type="submit"
              disabled={isSubmitting || updateMutation.isPending || selectedTests.length === 0 || offerPrice > totalPrice}
            ...
            >
              ...
              {isSubmitting || updateMutation.isPending ? "Updating Package..." : "Update Package"}
            </button>
          </div>
        </form>
      </div>
