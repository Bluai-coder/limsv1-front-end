      ...
      <div className="max-w-4xl mx-auto py-6">
        {/* Header */}
          ...
          <Link
            ...
            className="flex-shrink-0 p-2 sm:p-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800/60 transition-all duration-200 hover:scale-105 group mt-0.5 sm:mt-1"
          >
            <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 dark:text-gray-500 group-hover:text-gray-700 dark:group-hover:text-gray-300 transition-colors" />
          </Link>

          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center w-full gap-4 sm:gap-5">
            {/* Left Section - Title */}
            <div className="flex-1 min-w-0 w-full">
              <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">
                Register New Patient
              </h1>
              <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-0.5 sm:mt-1 flex items-center gap-2">
                {/* <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#1b4dff] dark:bg-[#3b6bff] opacity-70 dark:opacity-80 flex-shrink-0"></span> */}
                MRN will be auto-generated after registration
              </p>
            </div>

            {/* Right Section - Input & ABHA Link */}
            <div className="w-full lg:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-3 min-w-0 lg:min-w-[320px] xl:min-w-[400px]">
              {/* Input Container */}
              <div className="flex-1 min-w-[180px] sm:min-w-[200px] md:min-w-[220px]">
                <div className="relative">
                  <input
                    value={bluIdInput}
                    onChange={(e) => setBluIdInput(e.target.value)}
                    onKeyDown={handleBluIdKeyDown}
                    className="w-full px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/80 hover:bg-white dark:hover:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none focus:ring-4 focus:ring-[#1b4dff]/20 dark:focus:ring-[#3b6bff]/30 focus:border-[#1b4dff] dark:focus:border-[#3b6bff] transition-all duration-200 text-sm shadow-sm hover:shadow-md dark:shadow-gray-800/20 dark:hover:shadow-gray-800/30"
                    placeholder="Enter Patient BluId/ABHA ID"
                    onFocus={(e) => e.target.select()}
                  />
                  {isSearchingBluId && (
                    <div className="absolute inset-y-0 right-2.5 sm:right-3 flex items-center">
                      <Loader2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 animate-spin text-[#1b4dff] dark:text-[#3b6bff]" />
                    </div>
                  )}
                </div>
                {isSearchingBluId && (
                  <div className="flex items-center gap-2 mt-1.5 sm:mt-2 text-xs sm:text-sm text-gray-500 dark:text-gray-400 animate-pulse">
                    <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#1b4dff] dark:bg-[#3b6bff] animate-bounce"></span>
                    Fetching patient data...
                  </div>
                )}
              </div>

              {/* ABHA Register Button */}
              <Link
                href="https://bluhealth.bluai.ai/patient-registration/patient-abha-registration"
                target="_blank"
                className="flex-shrink-0 inline-flex items-center justify-center px-3 sm:px-4 md:px-5 py-2.5 sm:py-3 text-xs sm:text-sm font-medium text-[#1b4dff] dark:text-[#3b6bff] bg-[#1b4dff]/5 dark:bg-[#3b6bff]/10 hover:bg-[#1b4dff]/10 dark:hover:bg-[#3b6bff]/20 rounded-xl sm:rounded-2xl border border-[#1b4dff]/20 dark:border-[#3b6bff]/30 hover:border-[#1b4dff]/40 dark:hover:border-[#3b6bff]/50 transition-all duration-200 whitespace-nowrap hover:scale-[1.02] active:scale-[0.98] shadow-sm hover:shadow-md dark:shadow-gray-800/20 dark:hover:shadow-gray-800/30"
              >
                {/* <span className="hidden xs:inline"> </span> */}
                <span>Register for ABHA</span>
                <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 ml-1.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
